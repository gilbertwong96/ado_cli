defmodule AdoCli.CLI.TeamsTest do
  ...
  describe "list_teams" do
    test "halts 0 on successful get", %{server: server} do
      expect_success_json(server, "/test/_apis/teams", ~s({"value":[]}), fn ->
    ...
    test "halts 1 on API error", %{server: server} do
      expect_api_error(server, "/test/_apis/teams", 500, "{}", fn ->
        apply(AdoCli.CLI.Teams, :list_teams, [
          %{
            options: %{
              json: true,
  ...
  describe "show_team" do
    test "halts 0 on successful get", %{server: server} do
      expect_success_json(server, "/test/_apis/teams/1", ~s({"value":[]}), fn ->
        apply(AdoCli.CLI.Teams, :show_team, [
          %{
            options: %{json: true, expand_identity: false},
            arguments: %{project: "test", team_id: "1"}
          }
        ])
      end)
    end
    ...
    test "halts 1 on API error", %{server: server} do
      expect_api_error(server, "/test/_apis/teams/1", 500, "{}", fn ->
        apply(AdoCli.CLI.Teams, :show_team, [
          %{
            options: %{json: true, expand_identity: false},
            arguments: %{project: "test", team_id: "1"}
  ...
  describe "create_team" do
    test "halts 0 on successful post", %{server: server} do
      expect_post_success(server, "/test/_apis/teams", "", "{\"id\":1}", fn ->
  ...
  describe "update_team" do
    test "halts 0 on successful patch", %{server: server} do
      expect_patch_success(server, "/test/_apis/teams/1", "", "{\"id\":1}", fn ->
  ...
  describe "delete_team" do
    test "halts 0 on successful delete", %{server: server} do
      expect_delete_success(server, "/test/_apis/teams/1", fn ->
        apply(AdoCli.CLI.Teams, :delete_team, [
          %{options: %{json: true, force: false}, arguments: %{project: "test", team_id: "1"}}
        ])
      end)
    end
  ...
  describe "list_team_members" do
    test "halts 0 on successful get", %{server: server} do
      expect_success_json(server, "/test/_apis/teams/1/members", ~s({"value":[]}), fn ->
        apply(AdoCli.CLI.Teams, :list_team_members, [
          %{options: %{json: true, top: nil}, arguments: %{project: "test", team_id: "1"}}
        ])
      end)
    end

    test "halts 1 on API error", %{server: server} do
      expect_api_error(server, "/test/_apis/teams/1/members", 500, "{}", fn ->
        apply(AdoCli.CLI.Teams, :list_team_members, [
          %{options: %{json: true, top: nil}, arguments: %{project: "test", team_id: "1"}}
        ])
      end)
